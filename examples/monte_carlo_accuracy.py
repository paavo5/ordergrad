...
def _single_batch_estimates(
    ...
    *,
    ...
    x = to_backend(r[idx])
    v = to_numpy(os.orderstats(x))
    inc = to_numpy(os.orderstats_inclusion(x))
    adv = to_numpy(os.orderstats_advantage(x))
    l_inc = to_numpy(os_l.lstat_inclusion(x))
    l_adv = to_numpy(os_l.lstat_advantage(x))
...
def main() -> None:
    ...
    v_exact = to_numpy(os_exact.orderstats_known_rp(r, p))
    inc_exact_by_arm = to_numpy(os_exact.orderstats_inclusion_known_rp(r, p))
    adv_exact_by_arm = to_numpy(os_exact.orderstats_advantage_known_rp(r, p))
    l_inc_exact_by_arm = to_numpy(os_exact_l.lstat_inclusion_known_rp(r, p, a_backend))
    l_adv_exact_by_arm = to_numpy(os_exact_l.lstat_advantage_known_rp(r, p, a_backend))
